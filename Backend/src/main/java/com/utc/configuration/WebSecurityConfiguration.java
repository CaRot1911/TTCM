...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception{
        ...
        http.cors().and().authorizeHttpRequests().antMatchers(HttpMethod.GET,"UTCDemo/roomBook/**").hasAuthority("ADMIN")
                .antMatchers(HttpMethod.POST,"UTCDemo/roomBook/**").hasAnyAuthority("ADMIN","GUESTS")
                .antMatchers(HttpMethod.PUT,"UTCDemo/roomBook/**").hasAnyAuthority("ADMIN","GUESTS")
                .antMatchers(HttpMethod.DELETE,"UTCDemo/roomBook/**").hasAnyAuthority("ADMIN","GUESTS")
                .and()
                .exceptionHandling().authenticationEntryPoint(authEntryPoint)
                .and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                .and().httpBasic().and().csrf().disable();
